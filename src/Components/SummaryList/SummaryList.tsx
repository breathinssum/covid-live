...
const SummaryList: FunctionComponent<Props> = props => {
    ...
    const [sortOption, setSortOption] = useState<{
        ...
        // Omit : 타입스크립트의 유틸함수로 <가져올타입, 제외할 타입의 키값>
        //SUmmaryType에 cities란 타입을 제외한 키값의 타입을 얻기 위한 방법
    ...
    //useMemo : 계산된 값을 리턴하는 hook함수로 []안에 들어가는 값이 변경될 때마다 새로운 값을 리턴함
    //useState와 다른 점은 set하는 함수가 없고 자동으로 값이 계산되는 변수로 사용
    //sortOption과 props.summary의 값이 변경될 때 정렬값으로 테이브를 재정렬함
    const sortedCities = useMemo(() => {
        //prop.summary가 없는 경우 빈 배열 리턴
    ...
    //테이블 헤더를 클릭할 때 정렬 값을 변경하는 함수 key를 파라미터로 받고 클릭시 반대로 정렬
        ...
        <Box isPadding>
            <article className="summary-list">
                ...
                >
                    ...
                    <thead> //테이블 헤더 클릭시 정렬 값을 변경하는 함수 실행
                        // =) 정렬값이 변경되고 sortedCities의 useMemo가 다시 리턴되면서 테이블이 재졍렬됨
                    ...
                    <tbody> // 정렬된 도시 데이터를 테이블 뷰에 그리는 코드
                        //배열.map : 배열을 한바퀴 돌면서 리턴값에 따라 새로운 배열을 만듦
